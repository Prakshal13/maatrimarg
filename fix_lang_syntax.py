...
# First, remove the bad insertions. 
# We know they start with `    // FAQ Section` and end with `    faq_a5: ...`
cleaned_lines = []
skip = False
for line in lines:
    ...
    if skip and "faq_a5:" in line:
        skip = False # stop skipping after this line

# Now cleaned_lines has no FAQ keys at all.
# Let's write a function to insert them BEFORE the `  },` that precedes the next language block.
...
mr_faq = [
    ...
    '    faq_a5: "आमचे ML मॉडेल एक सहायक साधन म्हणून काम करते, डॉक्टरांना पर्याय म्हणून नाही. आम्ही \'AI ओव्हरराइड रेट\' (म्हणजे डॉक्टर AI चा निर्णय बदलू शकतात) कायम ठेवतो, ज्यामुळे वैद्यकीय सुरक्षितता सुनिश्चित होते आणि ग्रामीण तपासणीला वेग येतो.",\n'
]
mr_faq[-1] = mr_faq[-1].replace('\n', ',\n')
...
ta_faq[-1] = ta_faq[-1].replace('\n', ',\n')


def insert_before_lang(lines, lang_marker, new_lines):
    # Find the line index of `lang_marker`
    idx = -1
    for i, line in enumerate(lines):
        if line.startswith(lang_marker):
            idx = i
            break
            
    if idx == -1: return lines
    
    # Trace backwards to find the closing `  },` of the PREVIOUS language block.
    # Actually, if we just insert it before `  },` right above `lang_marker`, it's perfect.
    for j in range(idx-1, -1, -1):
        if lines[j].strip() == "},":
            # Ensure the line before it has a comma
            if not lines[j-1].strip().endswith(','):
                lines[j-1] = lines[j-1].rstrip('\n') + ',\n'
            return lines[:j] + new_lines + lines[j:]
            
    return lines

# en is before "  mr: {"
cleaned_lines = insert_before_lang(cleaned_lines, "  mr: {", en_faq)

# mr is before "  hi: {"
cleaned_lines = insert_before_lang(cleaned_lines, "  hi: {", mr_faq)

# hi is before "  ta: {"
cleaned_lines = insert_before_lang(cleaned_lines, "  ta: {", hi_faq)

# ta is before "};" at the end of the file.
for j in range(len(cleaned_lines)-1, -1, -1):
    if cleaned_lines[j].strip() == "};":
        # Trace back to find `  }`
        for k in range(j-1, -1, -1):
            if cleaned_lines[k].strip() == "}":
                if not cleaned_lines[k-1].strip().endswith(','):
                    cleaned_lines[k-1] = cleaned_lines[k-1].rstrip('\n') + ',\n'
                cleaned_lines = cleaned_lines[:k] + ta_faq + cleaned_lines[k:]
                break
        break
